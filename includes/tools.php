...
function render_tools_page() {
	...
	// Refresh Airstory connections.
	if ( isset( $_POST['_airstory_nonce'] ) && wp_verify_nonce( $_POST['_airstory_nonce'], 'refresh-connection' ) ) {
		Connection\trigger_connection_refresh( true, false );
	?>

		<div class="updated">
			<p><?php esc_html_e( 'Your site\'s connection(s) with Airstory are being updated.', 'airstory' ); ?></p>
		</div>
...
?>
	...
	<div class="wrap">
		...
		<form method="post">
			<h2><?php echo esc_html( _x( 'Airstory Connections', 'tools page heading', 'airstory' ) ); ?></h2>
			<p><?php esc_html_e( 'If you\'re having issues publishing from Airstory to WordPress, your connection may need to be refreshed.', 'airstory' ); ?></p>
			<?php submit_button( _x( 'Refresh connection', 'button label', 'airstory' ), 'secondary' ); ?>
			<?php wp_nonce_field( 'refresh-connection', '_airstory_nonce' ); ?>
		</form>
